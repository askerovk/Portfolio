...
- name: by Kamran Rasim Asgarov
  affiliation: 
abstract: "This paper describes the process of building a probabilistic English language model, capable of predicting words based on previous context. The model is trained on a corpus consisting of blog posts, online newspaper articles and twitter feeds. The basic model was smoothed by Kneser-Ney and pruned by Seymore-Rosenfeld methods. In addition, a simple Shiny application was built to showcase the model's capabilities. The app may be accessed [here](https://askerovk.shinyapps.io/5-grammodel). The end result is a 5-gram model, which recognizes over 40,000 words and 5,000,000 word combinations, is able to handle both unknown words and unknown combinations of words, and has a perplexity score of 253."
keywords: 
...
Lastly, a cutoff w.d.f. point is selected to reduce the table size by 95%, due to the a hard limit of 1gb RAM of the ShinyApp hosting server [^shiny].

[^shiny]: This project makes use of the free hosting package offered by https://www.shinyapps.io/. 
```{r}
...
The ultimate goal of this assignment was to create a Shiny application, which would allow the user to submit a word or a sequence of words and have the model predict the next word. The application itself may be viewed [here](https://askerovk.shinyapps.io/5-grammodel).

The basic operating principle of the app is as follows:

* Capture the last 4 words input by the user
* If there are less than 4 words, add "sss" tokens untill there are.
* Find all 5-grams with histories that match the 4 words.
* If there are 5 or more 5-grams, select the ones with highest probability and display their last words as predictions.
* If there are less than 5 5-grams, remove the first word from the 4 word sequence and match it with histories of 4-grams. 
* Repeat for 3-grams and 2-grams if necessary.
* If there are still less than 5 predictions by this point, supplement the difference with most likely unigrams. 

In order to speed up the calculations performed by the app, it helps to split up each ngram into history and prediction. The code below is the same for each of the ngram tables.  
...
Next comes user interface of the application, which consists of a set instructions for the app, a text field to input the text and 5 buttons, which show the words predicted by the model, from most to least likely.
...
shinyServer(function(input, output, session) {
    ...
    observeEvent(input$usertext, {
        ...
        ## Last 4 words are selected and de-captialized. 
        
        text <- char_tolower(text, 4)
        ...
        ## Keep the 10 most likely ngrams 
        ...
        data$predictions <- as.character(lapply(X = 1 : 5, 
                                                FUN = function(i){
        ...
        #### TRUNCATED: the code repeats for buttons 3-5 ####
        })
